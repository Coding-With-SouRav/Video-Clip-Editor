...
class VideoCutterApp:
    def __init__(self, root):
        ...
        self.undo_stack = []  # Stores states for undo
        self.redo_stack = []  # Stores states for redo
        self.max_undo_steps = 20  # Limit undo history


        self.playback_cursor = None
        self.timeline_update_running = False
        self.manual_cursor_time = None  # for manual click seek
        ...
        self.canvas.bind("<Button-3>", self.select_interval)  # right-click
    ...
    # NEW: Handle video end event
    def on_video_end(self, event):
        """Reset to beginning when video completes"""
        self.root.after(0, self.reset_to_beginning)
    
    def reset_to_beginning(self):
        """Reset playback to start position"""
        if self.vlc_player:
            self.vlc_player.stop()
    ...
    def draw_timeline(self):
        ...
        # Define how much width we want to allocate for the timeline line (black line)
        ...
        # Draw the shorter black timeline centered
    ...
    def update_timeline_cursor(self):
        ...
        # Check if current time is inside any deleted interval
        for start, end in self.deleted_intervals:
            if start <= current_time <= end:
                # Force skip to the end of the deleted interval + small buffer
        ...
        # Ensure virtual_time is valid and non-negative
    ...
    def to_virtual_time(self, real_time):
        """Convert real time (with deleted intervals) to virtual time on timeline."""
        virtual_time = real_time
        for start, end in self.deleted_intervals:
            if real_time > end:
                virtual_time -= (end - start)
            elif start <= real_time <= end:
                return None  # Inside a deleted interval
            else:
                break
        return virtual_time

    def to_real_time(self, virtual_time):
        """Convert virtual timeline position back to actual video time."""
    ...
    def ensure_playing(self):
        """Ensure VLC resumes playing after skipping an interval."""
    ...
    def save_video(self):
        ...
        self.cancel_export = False  # Reset flag before starting
        self.play_pause()
        self.controls_frame.pack_forget()
        self.controllar_frame.pack_forget()

        self.show_saving_animation()  # Show loader and cancel button
    ...
    def play_pause(self):
        ...
        if self.vlc_player:
            ...
            # If at the end of the video, reset to beginning
        ...
        if self.vlc_player:
            ...
            def finalize_seek():
                ...
                if was_playing:
                    ...
                    # Only forcibly pause if VLC tries to auto-play
                    if current_state == vlc.State.Playing:
                        self.vlc_player.pause()
                    self.play_pause_btn.config(image=self.play_icon)

            # Wait a short moment before finalizing to allow VLC to react
    ...
    def on_press(self, event):
        ...
        # Pause if we were paused before seeking
        if not was_playing:
            self.vlc_player.pause()
    ...
    def on_drag(self, event):
        if self.dragging_marker_idx is not None:
            ...
            # Prevent overlap with neighbor markers
    ...
    def on_window_resize(self, event):
        if event.widget == self.root:
            ...
            # Resize video output if needed (handled automatically by VLC in embedded frame)
            if platform.system() == "Windows":
                self.vlc_player.set_hwnd(self.video_frame.winfo_id())
            elif platform.system() == "Linux":
                self.vlc_player.set_xwindow(self.video_frame.winfo_id())
    ...
    # Add this helper method to restore a state
    ...
    # Add this method to push state to undo stack
    def push_undo_state(self):
        state = self.capture_state()
        if len(self.undo_stack) >= self.max_undo_steps:
            self.undo_stack.pop(0)  # Remove oldest state if over limit
        self.undo_stack.append(state)
        self.redo_stack = []  # Clear redo stack when new action is performed

    # Add this method to handle undo
